...
function CanvasBlocks(props: CanvasBlocks) {
  ...
  const optimizeStackHeight = (canvas: HTMLCanvasElement) => {
    // Compute number of rows and columns, and cell size
    let ratio = canvas.width / canvas.height;
    let ncols_float = Math.sqrt(numStacks * ratio);
    let nrows_float = numStacks / ncols_float;

    // Find best option filling the whole height
    let nrows1 = Math.ceil(nrows_float);
    let ncols1 = Math.ceil(numStacks / nrows1);
    while (nrows1 * ratio < ncols1) {
      nrows1++;
      ncols1 = Math.ceil(numStacks / nrows1);
    }
    let cell_size1 = canvas.height / nrows1;

    // Find best option filling the whole width
    let ncols2 = Math.ceil(ncols_float);
    let nrows2 = Math.ceil(numStacks / ncols2);
